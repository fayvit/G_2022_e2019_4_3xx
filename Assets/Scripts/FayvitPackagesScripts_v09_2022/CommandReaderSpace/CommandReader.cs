...
namespace FayvitCommandReader
{
    public static class CommandReader
    {
        ...
        /* Utilizado do antigo*/
        public static int InputDoControle(Controlador estava)
        {
            int retorno = (int)estava;
            for (int i = -1; i < 5; i++)
            {
                if (i!=0 && VerifyThisControlUse(i))
                    retorno = i;
            }
            return retorno;
        }
    ...
    public struct MsgChangeHardwareControler : IMessageBase
    {
        public Controlador newControler;
    }
}