    ...
    public class Multiplicar : PetAttackBase
    {
        ...
        void MultiplicarInsetos(GameObject G)
        {
            if (!addView && tempoDecorrido > gP.TempoDeInstancia)
            {
                ...
                GameObject G2 = InternalResourcesFolders.GetPet(C.MeuCriatureBase.NomeID);// GameController.g.El.retorna(C.MeuCriatureBase.NomeID);
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        Rigidbody r =  G2.AddComponent<Rigidbody>();
                        r.useGravity = false;
                        caps.isTrigger = true;
                        if (!controle)
